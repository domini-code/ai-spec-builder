...
function CopyButton({ spec, disabled }: { spec: Spec; disabled: boolean }) {
  ...
  async function handleCopy() {
    try {
      ...
      // clipboard write failed — no confirmation shown
    }
  }
  ...
  return (
    <button
      onClick={handleCopy}
      disabled={disabled}
      className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-600 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 transition disabled:opacity-60 disabled:cursor-not-allowed"
    >
      {copied ? (
        <>
...
export default function SpecOutput({ spec, isStreaming, onReset }: SpecOutputProps) {
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-gray-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <CopyButton spec={spec} disabled={isStreaming} />
          <button
            onClick={onReset}
